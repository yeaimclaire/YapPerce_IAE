...
const userServiceLink = new HttpLink({
  uri: process.env.NEXT_PUBLIC_USER_SERVICE_URL || 'http://localhost:4001/graphql',
});

const productServiceLink = new HttpLink({
  uri: process.env.NEXT_PUBLIC_PRODUCT_SERVICE_URL || 'http://localhost:4002/graphql',
});

const orderServiceLink = new HttpLink({
  uri: process.env.NEXT_PUBLIC_ORDER_SERVICE_URL || 'http://localhost:4003/graphql',
});

const paymentServiceLink = new HttpLink({
  uri: process.env.NEXT_PUBLIC_PAYMENT_SERVICE_URL || 'http://localhost:4004/graphql',
});
