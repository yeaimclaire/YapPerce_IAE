...
**GraphQL Endpoint**: `http://shipxpress:4005/graphql` (internal) atau `http://localhost:4005/graphql` (external)
...
**URL**: `http://order-service:4003/webhook/shipment-status` (internal) atau `http://localhost:4003/webhook/shipment-status` (external)
...
```bash
# Via GraphQL Playground di http://localhost:4003/graphql
mutation {
...
```bash
curl -X POST http://localhost:4003/webhook/shipment-status \
...
```bash
# Via GraphQL Playground di http://localhost:4005/graphql
query {
...
### Order Service
```env
SHIPXPRESS_URL=http://shipxpress:4005/graphql
```

### ShipXpress (untuk mengirim webhook)
```env
ORDER_SERVICE_WEBHOOK_URL=http://order-service:4003/webhook/shipment-status
```
